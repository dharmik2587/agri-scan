"""Pesticide lookup using Claude Sonnet 5. Accepts a product name or a photo of the
label/bottle and returns a comprehensive information & safety card."""
import os
import re
import json
import uuid
...
async def lookup_pesticide(
    ...
    chat = (
        LlmChat(
            ...
            session_id=f"pest-{uuid.uuid4().hex[:8]}",
    ...
    files = None
    if image_base64:
        clean, _ = _strip_data_uri(image_base64)
        files = [ImageContent(image_base64=clean)]

    try:
        response = await chat.send_message(UserMessage(text="\n".join(parts), file_contents=files or []))
    except Exception as e:
        logger.error("pesticide lookup LLM call failed: %s", e)
        raise
    ...
    try:
        ...
        logger.error("pesticide JSON parse failed: %s | raw: %s", e, raw[:400])
        data = {
